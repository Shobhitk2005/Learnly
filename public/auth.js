...
async function initializeFirebase() {
  try {
    ...
    showMessage('Failed to initialize authentication. Please refresh the page.', 'error');
  }
}
...
// Authentication state observer (called after Firebase initialization)
function initializeAuthStateListener() {
  onAuthStateChanged(auth, (user) => {
    ...
    } else if (user && (user.emailVerified || user.phoneNumber)) {
      // User is signed in and verified
      console.log('User is signed in and verified');
      // Don't redirect here - let the specific auth functions handle it
    } else {
      // User is signed out
